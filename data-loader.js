...
(function () {
  ...
  // If a renderer registers AFTER data has already loaded, it fires immediately.
  window.NCLTTracker = window.NCLTTracker || { renderers: [], data: null };

  // Replace renderers.push with a version that auto-fires post-load.
  (function () {
    var _push = window.NCLTTracker.renderers.push.bind(window.NCLTTracker.renderers);
    window.NCLTTracker.renderers.push = function (fn) {
      _push(fn);
      // If data has already arrived by the time this renderer registers,
      // call it now rather than waiting for an event that already fired.
      if (window.NCLTTracker.data) {
        try { fn(window.NCLTTracker.data); } catch (e) { console.error('[NCLT Tracker] Renderer error:', e); }
      }
    };
  })();
